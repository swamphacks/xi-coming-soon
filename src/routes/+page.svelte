			...
			<div class="flex flex-row gap-3 rounded p-6">
				...
				<form class="w-sm lg:w-auto"> 
					<input
						...
						id="email-input"
						class="w-sm mb-2 lg:w-md rounded border-2 border-b-4 border-input-border bg-input p-4 text-xl lg:text-2xl font-bold text-input-text focus:border-input-focus-border"
						placeholder="email"
					/>
					<Button on:click={sendInterestRequest} class="text-xl lg:text-2xl">Subscribe</Button>
				</form>
			</div>
			{#if inputError}
